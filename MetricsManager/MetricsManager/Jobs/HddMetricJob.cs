...
namespace MetricsManager.Jobs
{
    ...
    public class HddMetricJob : IJob
    {
        ...
        public HddMetricJob(IHddMetricsRepository hddMetricsRepository, ILogger<HddMetricJob> logger,
        ...
        public Task Execute(IJobExecutionContext context)
        {
            ...
            if (agents.Any())
            {
                foreach (var agent in agents)
                {
                    try
                    {
                        var metrics = _metricsAgentClient.GetAllHddMetrics(new GetHddMetricsApiRequestByTimePeriod
                        {
                            AgentUrl = agent.AgentUrl,
                            FromTime = _hddMetricsRepository.GetLastRecordTimeByAgentId(agent.AgentId),
                        ...
                        _hddMetricsRepository.Create(metricForManagerDB);
                    }
                    catch (Exception ex)
                    {
